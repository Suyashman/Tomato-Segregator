...
# ---------------- CONFIG ----------------
CAMERA_INDEX = 1              # <-- your laptop/USB cam index
ARDUINO_IP = "192.168.xxx.xxx" #<-- change xx to your Arduino's IP
MODEL_PATH = r"best.pt" #<-- your model path

CONF_THRESHOLD = 0.5
FRAME_SKIP = 1                # we can process every frame now
COOLDOWN = 2.5
PERSIST_FRAMES = 4
SIZE_THRESHOLD = 39000         # area threshold for Task 4
...
def get_current_task(arduino_ip):
    ...
    try:
        ...
        pass
    print("⚠️ Arduino unreachable, defaulting to Task 1")
    return 1
...
while True:
    ret, frame = cap.read()
    if not ret: 
        print("⚠️ Frame missing"); continue
    ...
    if results[0].boxes:
        ...
        if conf > CONF_THRESHOLD:
            ...
            elif CURRENT_TASK == 4:             # Small / Large
    ...
    cv2.imshow("Tomato Segregator (Wired Cam)", annotated)
